$(function()
{$("input,textarea").jqBootstrapValidation({preventSubmit:!0,submitSuccess:function($form,event)
{if(!$form.attr('action'))
...
formData[fieldID]=fieldData});$.ajax({url:processorFile,type:"POST",data:formData,cache:!1,success:function()
...
{$form.append("<div id='form-alert'><div class='alert alert-danger'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button><strong>"+$form.attr('data-fail-msg')+"</strong></div></div>")}},})}},filter:function()
